...
SR_PRIV int rs_fsw_and_fsv_local(const struct sr_dev_inst *sdi)
{
	...
	/* disable display power save mode*/
	...
	double *min_ref_level, double *max_ref_level)
{
	...
	return sr_scpi_get_double(scpi, "DISP:TRAC:Y:RLEV? MAX", max_ref_level);
}
	...
	double frequency)
{
	...
	return rs_fsw_and_fsv_read_span(sdi);
}
...
SR_PRIV int rs_fsw_and_fsv_set_span(const struct sr_dev_inst *sdi, double span)
{
	...
	return rs_fsw_and_fsv_read_frequency(sdi);
}
...
SR_PRIV int rs_fsw_and_fsv_set_rbw(const struct sr_dev_inst *sdi, uint64_t rbw)
{
	...
	return rs_fsw_and_fsv_read_vbw(sdi);
}
...
SR_PRIV int rs_fsw_and_fsv_set_vbw(const struct sr_dev_inst *sdi, uint64_t vbw)
{
	...
	return rs_fsw_and_fsv_read_rbw(sdi);
}
	...
	double ref_level)
{
	...
	ret = rs_fsx_scpi_send_rwlocked(sdi, "DISP:TRAC:Y:RLEV %fdBm",
		devc->ref_level);
	if (ret == SR_OK)
	...
	size_t idx)
{
	...
	ret = rs_fsx_scpi_send_rwlocked(sdi, "ROSC:SOUR %s",
		(idx == 0) ? "INT" : "EXT1");
	if (ret == SR_OK)
...
SR_PRIV int rs_fsw_and_fsv_receive_data(int fd, int revents, void *cb_data)
{
	...
	ret = rs_fsw_and_fsv_read_num_points(sdi);
	if (ret != SR_OK)
		return TRUE;
