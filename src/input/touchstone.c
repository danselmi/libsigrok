...
static int prepare_data_set_memory(struct context *inc, size_t min_cap)
{
	...
	{
		double *ptr;
		ptr = g_malloc((inc->data_set_size + INITIAL_DATA_SET_SIZE) *
			sizeof(double));
		if (!ptr){
			sr_err("failed to alloc memory");
			return SR_ERR;
...
static int prepare_sweep_mem(struct context *inc)
{
	...
	if (inc->sweep_size == 0) {
		...
		inc->sweep_data = g_malloc(sizeof(double) * sweep_points *
			data_set_entries);
		...
		memcpy(ptr_d, inc->sweep_data, inc->sweep_count*sizeof(double) *
			data_set_entries);
...
static int move_data_to_sweep(struct context *inc)
{
	...
	else
	{
		...
		memcpy(ptr, &inc->data_set[1], (inc->num_vals_per_set-1) *
			sizeof(double));
		/* data_set[0] Frequency in units.
		 * data_set[1] Minimum noise figure in dB.
		 * data_set[2] Source reflection coefficient to realize minimum noise
			figure (MA).
	...
	if (inc->file_version == 1 && inc->state == PS_DATA_LINES && ret == SR_OK) {
		if (inc->sweep_count && inc->data_set_count) {
			if (inc->last_freq >= inc->data_set[0]) {
				...
				inc->sweep_size = inc->sweep_size * inc->num_ports *
					inc->num_ports * 2 / 5;
